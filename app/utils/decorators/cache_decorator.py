    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if "validation_error" in result_json:
                return jsonify({"error": "Validation error", "msg": [err.get("msg") for err in result_json["validation_error"]]}), 400
            if "result" not in result_json:
                return jsonify({"error": f"Missing 'result' key in response {result_json}"}), 500
            if "string" not in result_json["result"]:
                return jsonify({"error": f"Missing 'string' key in 'result' {result_json}"}), 500
            if "scientific" not in result_json["result"]:
                return jsonify({"error": f"Missing 'scientific' key in 'result' {result_json}"}), 500
